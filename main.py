...
def run():
    ...
    pedestrian_process = Processor(frame_P, detectorP, tracker, droi_P, show_droi_P, mcdf,
                                mctf, detection_interval, counting_lines_P, speed_estimation_lines, show_counts_P, frames_P_processed, processing_frame_rate, roads)
    ...
    while is_cam or cap.get(cv2.CAP_PROP_POS_FRAMES) + 1 < cap.get(cv2.CAP_PROP_FRAME_COUNT) or cap_P.get(cv2.CAP_PROP_POS_FRAMES) + 1 < cap_P.get(cv2.CAP_PROP_FRAME_COUNT):
        ...
        if line_setting_mode_P is True:
            ...
            line_setting_mode = False
            if _len >= 4:
                 settings.replace_line('.env', 24, "COUNTING_LINES_P=[{'label': 'A', 'line':"+ str(ped_coordinate.points[0:3]) + "}, {'label': 'B', 'line':"+ str(ped_coordinate.points[-2:])+"}]\n") 
                 os.putenv('COUNTING_LINES_P',"[{'label': 'A', 'line':"+ str(ped_coordinate.points[0:3]) + "}, {'label': 'B', 'line':"+ str(ped_coordinate.points[-2:])+"}]")
                 os.execl(sys.executable, os.path.abspath(__file__), *sys.argv)           
        ...
        if ret:
            ...
            output_frame = vehicle_process.visualize(roi_setting_mode, is_paused, showUI)
        ...
        if ret_P:
            ...
            output_P_frame = pedestrian_process.visualize(roi_setting_mode_P, is_paused, showUI)
